<template>
  <section class="add">
    ...
    <el-form-item>
        <button @click='getResult()' v-trigger>推荐后门植入方法</button>
      
    </el-form-item>
    <div>
    <a :href="'http://localhost:8080/#/HMZR func?name='+form.test[form.index]">{{form.test[form.index].substr(0,15)}}</a>
  </div>
  <h>
   ------------------------------------------
  </h>
  <div>
    <a :href="'http://localhost:8080/#/HMZR func?name='+form.test[form.index+1]">{{form.test[form.index+1].substr(0,15)}}</a>
  </div>
   <h>
   ------------------------------------------
  </h>
  <div>
    <a :href="'http://localhost:8080/#/HMZR func?name='+form.test[form.index+2]">{{form.test[form.index+2].substr(0,15)}}</a>
  </div>
   <h>
   ------------------------------------------
  </h>
  <div>
    <a :href="'http://localhost:8080/#/HMZR func='+form.test[form.index+3]">{{form.test[form.index+3].substr(0,15)}}</a>
  </div>
  <h>
   ------------------------------------------
  </h>
  <div>
    <a :href="'http://localhost:8080/#/HMZR func?name='+form.test[form.index+4]">{{form.test[form.index+4]}}</a>
  </div>
    <h>
   ------------------------------------------
  </h>
...
directives:{
  ...
  methods: {
    ...
    getResult:function() {
        var _this = this
        _this.$axios.get('http://127.0.0.1:5000/api/hm/?name='+_this.form.toolname).then
        (function(resp){
